...
function normalizeScriptHash(value: string): string {
...
function emit(opcode: OpCode, operand: Uint8Array = new Uint8Array()): Uint8Array {
  return concatBytes(Uint8Array.of(opcode), operand);
}

function emitPushInt(value: bigint): Uint8Array {
  if (value >= -1n && value <= 16n) {
    return emit((OpCode.PUSHM1 + Number(value) + 1) as OpCode);
  }
...
function emitPushBytes(value: Uint8Array): Uint8Array {
...
function emitSyscall(syscallName: string): Uint8Array {
...
export function normalizePublicKey(value: string, compressed = true): string {
  const normalized = normalizeHex(value);
...
export function isWIF(value: string): boolean {
  try {
...
export function publicKeyFromPrivateKey(privateKey: string, compressed = true): string {
  const normalized = normalizePrivateKey(privateKey);
  return bytesToHex(p256.getPublicKey(hexToBytes(normalized), compressed));
}

export function signHex(hex: string, privateKey: string): string {
...
export function signBytes(message: Uint8Array, privateKey: string): Uint8Array {
...
export function verifyHex(hex: string, signature: string, publicKey: string): boolean {
...
export function verifyBytes(message: Uint8Array, signature: Uint8Array, publicKey: string): boolean {
  const digest = sha256Bytes(message);
...
export function getAddressFromScriptHash(scriptHash: string, addressVersion = DEFAULT_ADDRESS_VERSION): string {
...
export function constructMultiSigVerificationScript(signingThreshold: number, publicKeys: string[]): string {
  ...
  const chunks: Uint8Array[] = [emitPushInt(BigInt(signingThreshold))];
...
export function scriptHashToAddressLabel(scriptHash: string): { scriptHash: string; address: string } {
  const normalized = normalizeScriptHash(scriptHash);
  return {
...
export function addressToScriptHashLabel(address: string): { scriptHash: string; address: string } {
  const scriptHash = getScriptHashFromAddress(address);
  return { address, scriptHash };
}

export function verificationScriptToBase64(publicKey: string): string {
  return bytesToBase64(hexToBytes(getVerificationScriptFromPublicKey(publicKey)));
}

export function isSupportedAccountInput(value: string): boolean {
...
export function hash160Script(scriptHex: string): string {
