...
type _InvokeInvocationTree = Assert<IsEqual<NonNullable<InvokeDiagnosticsResult["invocations"]>[number], InvokeDiagnosticsInvocationTree>>;
type _InvokeStorageChange = Assert<IsEqual<NonNullable<InvokeDiagnosticsResult["storagechanges"]>[number], InvokeDiagnosticsStorageChange>>;
type _PendingSignature = Assert<IsEqual<NonNullable<InvokeResult["pending_signature"]>, PendingSignatureResult>>;
type _PendingSignatureItem = Assert<IsEqual<PendingSignatureResult["items"][number], PendingSignatureContextItem>>;
...
type _RelayTxHash = Assert<IsEqual<RelayTransactionResult["hash"], string>>;
...
const getBlockVerbose = (client: RpcClient) => client.getBlock({ indexOrHash: "0x1", verbose: true });
const getBlockRaw = (client: RpcClient) => client.getBlock({ indexOrHash: "0x1", verbose: false });
const getBlockHash = (client: RpcClient) => client.getBlockHash({ blockIndex: 1 });
const getBlockHeaderVerbose = (client: RpcClient) => client.getBlockHeader({ indexOrHash: "0x1", verbose: true });
const getBlockHeaderRaw = (client: RpcClient) => client.getBlockHeader({ indexOrHash: "0x1", verbose: false });
const getRawTransactionVerbose = (client: RpcClient) => client.getRawTransaction({ hash: "0x1", verbose: true });
const getRawTransactionRaw = (client: RpcClient) => client.getRawTransaction({ hash: "0x1", verbose: false });
...
const getStateRoot = (client: RpcClient) => client.getStateRoot({ index: 1 });
const getTransactionHeight = (client: RpcClient) => client.getTransactionHeight({ hash: "0x1" });
...
type _GetBlockHash = Assert<IsEqual<ReturnType<typeof getBlockHash>, Promise<string>>>;
...
type _GetStateRootObject = Assert<IsEqual<ReturnType<typeof getStateRoot>, Promise<GetStateRootResult>>>;
type _GetTransactionHeight = Assert<IsEqual<ReturnType<typeof getTransactionHeight>, Promise<number>>>;
